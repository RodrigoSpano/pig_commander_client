...
export default function () {
  ...
  const handleChange = (e) => {
    ...
    const shouldParseIntValue = ["category_id", "method_id"].includes(
      fieldName
    );
  ...
  const handleSubmitIncome = useCallback(
    (event) => {
      event.preventDefault();
      const { token } = cookies;
      dispatch(
  ...
  const isDate =
    automatizedForm.auto_date > 0 && automatizedForm.auto_date <= 31;
  ...
  const someFieldEmpty =
    !isName || !isAmount || !formWallet.category_id || !formWallet.method_id;
