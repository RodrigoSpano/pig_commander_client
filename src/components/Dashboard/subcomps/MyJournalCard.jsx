...
export default function MyJournalCard() {
  ...
  return (
    <motion.div className="p-6 bg-white rounded-lg shadow-md mb-4 select-none"
    ...
    >
      ...
      <div className="custom-scrollbar grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-96 overflow-y-scroll">
        {journal.slice(0, visibleNewsCount).map((element, index) => (
          <motion.div key={index} className="border border-gray-300 p-3 rounded-lg"
            whileHover={{ scale: 1.02, boxShadow: "0px 2px 4px rgba(0, 0, 0, 0.1)" }}
          >
            <h3 className="text-lg font-semibold mb-1">{element.title}</h3>
            <img src={element.banner_image} alt={element.title} className="mb-1" style={{ maxWidth: "100%" }} />
            <p className="text-gray-600 text-sm mb-1">{element.summary}</p>
            <p className="text-xs text-gray-400">Author: {element.author}</p>
            <a href={element.url} target="_blank" rel="noopener noreferrer" className="text-regularPink hover:underline text-xs">
              Read more
            </a>
          </motion.div>
        ))}
