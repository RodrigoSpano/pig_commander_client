    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center p-4">
                ...
                <h1 className="text-2xl font-bold my-4 dark:text-mediumPinkDark">Formulario de transacción</h1>

                <div className="flex flex-row items-center">
                    <Input autoComplete="off" type="text" name="name" label="Nombre" size="sm" value={formWallet.name} onChange={handleChange} onFocus={allowNameErrorMessage} className="mr-2" />

                    <Input autoComplete="off" type="number" name="amount" label="Cantidad" size="sm" value={formWallet.amount} onChange={handleChange} onFocus={allowAmountErrorMessage} className="ml-2" />
                </div>
                ...
                <div className="mt-5">
                    {
                        automatized
                            ...
                            : (
                                <>
                                    <button className={`text-white h-10 w-32 font-bold cursor-no-drop mr-5 rounded-2xl mt-8 text-base ${someFieldEmpty ? "bg-regularGray" : " cursor-pointer bg-gradient-to-r from-regularPink  to-boldPink"} `} disabled={someFieldEmpty} onClick={handleSubmitIncome}>Agregar Gasto</button>
                                    <button className={`text-white h-10 w-32 font-bold cursor-no-drop rounded-2xl mt-8 text-base ${someFieldEmpty ? "bg-regularGray" : " cursor-pointer bg-gradient-to-r from-regularPink  to-boldPink"} `} disabled={someFieldEmpty} onClick={handleSubmitExpense}>Agregar Ingreso</button>
                                </>
                            )
                    }
