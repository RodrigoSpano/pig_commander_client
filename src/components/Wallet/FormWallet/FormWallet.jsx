  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center p-4">
        <h1 className="text-2xl font-bold my-4 dark:text-mediumPinkDark">
          Transaction Form
        </h1>
        ...
        <div className="flex flex-row sm:flex-col md:flex-row justify-center items-center">
          <div className="flex flex-col mr-2">
            <label className="text-boldGray text-lg font-light dark:text-white">
              Method:
            </label>
            <select
              className="text-md w-40 dark:text-black  bg-lightGray font-light rounded-lg p-1.5 h-12 cursor-pointer"
              name="method_id"
              id="selects"
              onChange={handleChange}
              value={formWallet.method_id}
            >
              <option label="Choose a Method" value="" disabled hidden />
              {methods.length
                ? methods.map((method) => (
                    <option
                      key={method.id}
          ...
          <div className="flex flex-col ml-2">
            ...
            <select
              className="text-md capitalize w-40 dark:text-black  bg-lightGray font-light rounded-lg p-1.5 h-12 cursor-pointer"
              name="category_id"
              id="selects"
              value={formWallet.category_id}
              onChange={handleChange}
            >
              <option label="Choose a category" value="" disabled hidden />
              {categories.length
        ...
        <div className="flex mt-2">
          ...
          <span className="text-boldGray text-md font-light mt-5 dark:text-white">
            Automatized: Only premium members
          </span>
        </div>
        ...
        <div className="flex flex-row sm:flex-col md:flex-row items-center justify-center mt-5">
          <Switch
            ...
            size="sm"
            color="success"
            isSelected={automatized}
            onValueChange={setAutomatized}
            isDisabled={!userPremium.premium}
          />
          <input
            className="bg-lightGray dark:text-black  placeholder-black font-light rounded-lg text-xs p-1.5 w-40 relative focus:outline-none active:outline-none"
            type="number"
            placeholder="Day of the month..."
            name="auto_date"
            value={!automatizedForm.auto_date}
            disabled={!automatized}
            onChange={handleAutoChange}
          />
          <select
            defaultValue={"expense"}
            name="type"
            className="text-xs w-26 bg-lightGray dark:bg-white dark:text-black font-light rounded-lg p-1.5 ml-2"
            disabled={!automatized}
            onChange={handleAutoChange}
          >
        ...
        <div className="mt-5">
          {automatized ? (
            <div className="flex flex-col">
              <button
                className={`text-white h-10 self-center w-32 font-bold cursor-no-drop mr-5 rounded-2xl mt-8 text-base ${
                  someFieldEmptyAutomatized
                    ? "bg-regularGray"
                    : " cursor-pointer bg-gradient-to-r from-regularPink  to-boldPink"
                } `}
                disabled={someFieldEmptyAutomatized}
                onClick={handleSubmitAutomatize}
              >
                Automatize
              </button>
              <ModalAutomatized />
